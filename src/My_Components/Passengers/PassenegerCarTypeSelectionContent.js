      ...
      try {
        ...
        if (res.status === 200) {
          ...
          if (oneWayTrip) {
            ...
            console.log("🛑 Navigating to Round Trip Confirmation Page...");
            toast.success(
              `Your round trip booking has been confirmed for ${format(
      ...
      <div className="w-full max-w-2xl overflow-hidden bg-white shadow-lg rounded-lg">
        ...
        <div className="p-6">
          <motion.form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {carOptions.map((car) => (
                  <div
                    key={car.type}
                    className={`flex items-center justify-between p-4 rounded-lg border border-gray-200 hover:border-[#0bbfe0] transition-colors ${selectedCar === car.type ? "bg-[#e0f2f7]" : ""
                      }`}
                  >
                    <input
                      type="radio"
            ...
            <button
              type="submit"
              className={`flex items-center justify-center w-auto bg-[#0bbfe0] hover:bg-[#0999b3] text-white px-4 py-2 rounded-md ${!selectedCar ? "opacity-50 cursor-not-allowed" : ""
                }`}
              disabled={!selectedCar}
            >
              Book Your Trip <ArrowRight className="ml-2 h-4 w-4" />
